...
function applicationToCSVRow(app: any, origin: string): string[] {
  ...
  row.push(escapeCSV(app.preferred_job_role ? JSON.stringify(app.preferred_job_role) : null));
  row.push(escapeCSV(app.preferred_job_location ? JSON.stringify(app.preferred_job_location) : null));
  ...
  row.push(escapeCSV(app.education ? JSON.stringify(removeIdFields(app.education)) : null));
  row.push(escapeCSV(app.work_experience ? JSON.stringify(removeIdFields(app.work_experience)) : null));
  row.push(escapeCSV(app.projects ? JSON.stringify(removeIdFields(app.projects)) : null));
  row.push(escapeCSV(app.published_papers ? JSON.stringify(removeIdFields(app.published_papers)) : null));
  row.push(escapeCSV(app.technical_skills ? JSON.stringify(removeIdFields(app.technical_skills)) : null));
  row.push(escapeCSV(app.languages ? JSON.stringify(removeIdFields(app.languages)) : null));
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (invalidIds.length > 0) {
      return NextResponse.json({ error: "Invalid UUIDs", invalidIds }, { status: 400 });
    }
    ...
    console.info(`CSV export complete: ${applications.length} applications exported`);
